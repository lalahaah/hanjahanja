...
반드시 다른 설명 없이 아래의 JSON 구조로만 응답해주세요:
{
  ...
  "level": "이 단어를 구성하는 한자 중 가장 어려운 한자의 한국어문회 한자능력검정시험 급수를 다음 중 하나로만: 8급, 7급Ⅱ, 7급, 6급Ⅱ, 6급, 5급Ⅱ, 5급, 4급Ⅱ, 4급, 3급Ⅱ, 3급, 2급, 1급, 특급Ⅱ, 특급"
}

규칙:
- relatedWords는 2~3개, 초등학생이 알 만한 실생활 단어로 골라주세요.
- idioms는 해당 한자를 포함하는 사자성어가 있으면 1~2개, 적절한 것이 없으면 빈 배열 []로 응답하세요. 억지로 만들어내지 마세요.
- level은 정확한 공인 급수를 모르겠으면 한자의 상용 빈도와 난이도로 가장 근접하게 추정한 급수를 답해주세요. 절대 비워두지 마세요.
`.trim();
